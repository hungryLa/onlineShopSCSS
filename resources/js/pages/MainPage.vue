...
import SliderCard from "@/components/client/product/SliderCard.vue";
...
const getProducts = async () => {
    ...
    return all.data.data.reduce((result:Product[][], item:Product, index:number) => {
...
const handleScroll = () => {
    const searchRef = ref<any>(document.body)
    let {top} = searchRef.value.getBoundingClientRect()

    if(top < -353) headerStore.headerRef.classList.remove('header-hidden');
    if(top > -352) headerStore.headerRef.classList.add('header-hidden');
}
...
.running_slider {
    ...
    &__search {
        ...
        &__spot {
            ...
            z-index:-1;filter:blur(41.5px);background:radial-gradient(50% 50% at 50% 50%, color(display-p3 1 1 1) 55.89%, color(display-p3 1 1 1 / 0.00) 100%)
        }
        &__svg {
            @apply text-[#5433eb];
        }
